...
from src.stock_fetcher import (fetch_stock_info, fetch_stock_price, fetch_stock_ohlcv,
                               ...
                               fetch_finbox_forward_fcf_yield, fetch_gurufocus_forward_fcf_yield)
...
@st.cache_data(ttl=3600)
def _finbox_fcf_fwd(ticker):
    return fetch_finbox_forward_fcf_yield(ticker)

@st.cache_data(ttl=3600)
def _gurufocus_fcf_fwd(ticker):
    return fetch_gurufocus_forward_fcf_yield(ticker)
...
def _stock_metric_cards(info: dict, fcf_yield, curr_rsi_d, curr_rsi_w, fcf_fwd_fb=None, fcf_fwd_gf=None) -> None:
...
<div class="metric-grid">
  ...
  <div class="mgroup" style="--gc:#4CAF50">
    ...
    <div class="mpair" style="margin-top:8px; padding-top:8px; border-top:1px solid rgba(255,255,255,0.1)">
      <div class="mitem">
        <div class="mlabel">FCF Yield (2026F - Finbox)</div>
        <div class="mval">{v(fcf_fwd_fb,'.1f',suf='%') if fcf_fwd_fb is not None else 'N/A'}</div>
      </div>
      <div class="mitem">
        <div class="mlabel">FCF Yield (2026F - GuruFocus)</div>
        <div class="mval">{v(fcf_fwd_gf,'.1f',suf='%') if fcf_fwd_gf is not None else 'N/A'}</div>
      </div>
    </div>
  </div>
...
def render_single_stock(ticker: str) -> None:
    with st.spinner("Loading stock data..."):
        ...
        fcf_fwd_fb = _finbox_fcf_fwd(ticker)
        fcf_fwd_gf = _gurufocus_fcf_fwd(ticker)
        ...
        _stock_metric_cards(info, fcf_yield, curr_rsi_d, curr_rsi_w, fcf_fwd_fb, fcf_fwd_gf)
